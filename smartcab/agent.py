...
class LearningAgent(Agent):
	...
	def choose_action(self, state):
		...
		action_with_maximum_Q_value = max(possible_actions.items(), key=lambda x: x[1])[0]
		if self.learning:
			value_using_epsilon  = random.random() < 1 - self.epsilon
			if not value_using_epsilon:
				valid_actions = filter(lambda x: x != action_with_maximum_Q_value, Environment.valid_actions)
				action = random.choice(valid_actions)
			else:
				action = action_with_maximum_Q_value
		else:
